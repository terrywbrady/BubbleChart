...
var initStr_test = "Sleep;;Slee;;56;;-1;;-1;;Family;;Fami;;20;;-2;;-2;;Meals;;Meal;;15;;-3;;-3;;Fitness;;Fitn;;12;;-4;;-4;;";
var initStr = "";
...
function parseParam() {
	...
	if (s==null) s = "";
	var i = s.indexOf("data=");
	if (i == -1) {
		return initStr;
	}
	s = s.substring(i+5,s.length);
	return s;
}

function initFromParam(s) {
	var arr = s.split(";;");
	var c = (arr.length-1) / 5;
	
	for(var i=0; i<c; i++) {
		var abbrev = arr[i*5+1];
		var necessity = Number(arr[i*5+3]);
		var fulfillment = Number(arr[i*5+4]);
		var name = arr[i*5];
		var hours = Number(arr[i*5+2]);
		
		var activity = activityList.addActivity(name, hours);
		activity.fulfillment = fulfillment;
		activity.necessity = necessity;
	}
...
ActivityList.prototype.drawTable = function(summary) {
	...
	if (summary) {
		...
		if (this.list.length > 0) {
			...
			drawPie('actchart');
			var v1 = this.drawImportTable();
			var v2 = this.drawFulfillmentTable();
...
ActivityList.prototype.drawRankTable = function(tableId, field, topHeader, bottomHeader) {
	...
	var list = this.list.sort(compare);
